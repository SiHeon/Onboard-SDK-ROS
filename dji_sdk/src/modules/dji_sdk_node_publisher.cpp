...
//  ros::Time current_time;
  ...
  Telemetry::TypeMap<Telemetry::TOPIC_GPS_FUSED>::type synced_global_pos =
    vehicle->subscribe->getValue<Telemetry::TOPIC_GPS_FUSED>();

  Telemetry::TypeMap<Telemetry::TOPIC_VELOCITY>::type v_FC =
    vehicle->subscribe->getValue<Telemetry::TOPIC_VELOCITY>();

  sensor_msgs::NavSatFix synced_gps_pos;
  synced_gps_pos.latitude        = synced_global_pos.latitude * 180 / C_PI;
  synced_gps_pos.longitude       = synced_global_pos.longitude * 180 / C_PI;
  synced_gps_pos.altitude        = synced_global_pos.altitude;
  p->current_gps_latitude = synced_gps_pos.latitude;
  p->current_gps_longitude = synced_gps_pos.longitude;
  p->current_gps_altitude = synced_gps_pos.altitude;
  ...
  if(p->local_pos_ref_set)
  {
           ...
           double y;
           double z;
           p->gpsConvertENU(x, y, synced_gps_pos.longitude, synced_gps_pos.latitude, p->local_pos_ref_longitude, p->local_pos_ref_latitude);
           z = synced_gps_pos.altitude - p->local_pos_ref_altitude;
...
//           ros::spinOnce();
               ...
               tf::StampedTransform(
               tf::Transform(tf::Quaternion(q_FLU2ENU.getX(), q_FLU2ENU.getY(), q_FLU2ENU.getZ(), q_FLU2ENU.getW()),
                             tf::Vector3(x, y, z)),
               msg_time,"odometry", "velo"));

               nav_msgs::Odometry synced_odometry;

               synced_odometry.header.stamp = msg_time;
               synced_odometry.header.frame_id = "odometry";
               synced_odometry.child_frame_id = "velo";
               p->gpsConvertENU(synced_odometry.pose.pose.position.x, synced_odometry.pose.pose.position.y, synced_gps_pos.longitude,
                                synced_gps_pos.latitude, p->local_pos_ref_longitude, p->local_pos_ref_latitude);
               synced_odometry.pose.pose.position.z = synced_gps_pos.altitude - p->local_pos_ref_altitude;
               synced_odometry.pose.pose.orientation.x = q_FLU2ENU.getX(); //q_FLU2ENU.getX();
               synced_odometry.pose.pose.orientation.y = q_FLU2ENU.getY(); //q_FLU2ENU.getY();
               synced_odometry.pose.pose.orientation.z = q_FLU2ENU.getZ(); //q_FLU2ENU.getZ();
               synced_odometry.pose.pose.orientation.w = q_FLU2ENU.getW(); //q_FLU2ENU.getW();
               synced_odometry.twist.twist.angular.x =  hardSync_FC.w.x; //vehicle->broadcast->getAngularRate().x;
               synced_odometry.twist.twist.angular.y = -hardSync_FC.w.y; //-vehicle->broadcast->getAngularRate().y;
               synced_odometry.twist.twist.angular.z = -hardSync_FC.w.z; //-vehicle->broadcast->getAngularRate().z;
               synced_odometry.twist.twist.linear.x = v_FC.data.y; //vehicle->broadcast->getVelocity().y;
               synced_odometry.twist.twist.linear.y = v_FC.data.x; //vehicle->broadcast->getVelocity().x;
               synced_odometry.twist.twist.linear.z = v_FC.data.z; //vehicle->broadcast->getVelocity().z;

               p->odometry_publisher.publish(synced_odometry);
